...
export default function PatientPortal() {
  ...
  // Use record_type (Supabase field)
  const filtered =
    activeTab === "all"
      ? records
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        >
          <Shield size={16} />
          Your health records are managed by your hospital. You have read-only
          access.
        </div>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                ...
                <span className="text-sm text-muted">
                  DOB:{" "}
                  {p?.dateOfBirth
                    ? safeFormat(p.dateOfBirth, "dd MMM yyyy")
                    : "—"}
                </span>
        ...
        {(p?.bloodGroup || p?.genotype) && (
          ...
          >
            ...
            <span style={{ color: "var(--text-2)" }}>
              Emergency info — Blood:{" "}
              <strong style={{ color: "var(--text-1)" }}>
                {p?.bloodGroup}
              </strong>
              &nbsp;· Genotype:{" "}
              <strong style={{ color: "var(--text-1)" }}>{p?.genotype}</strong>
            </span>
          </div>
        )}
        ...
        <div className="card">
          <div className="card-header">
            <h3 className="card-title">My Medical Records</h3>
          </div>
              ...
              >
                ...
                {t.key !== "all" && (
                  <span style={{ marginLeft: 4, opacity: 0.7 }}>
                    ({records.filter((r) => r.record_type === t.key).length})
                  </span>
                )}
              </button>
            ))}
...
function RecordItem({ record, isExpanded, onToggle }) {
  ...
  const color = RECORD_COLORS[record.record_type] || "var(--text-2)";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div
            style={{ color: "var(--text-1)", fontWeight: 500, fontSize: 14 }}
          >
            {record.title}
          </div>
          <div style={{ fontSize: 12, color: "var(--text-3)", marginTop: 2 }}>
            {record.hospital_name || "—"} · {record.doctor_name || "—"} ·{" "}
            {safeFormat(record.created_at, "dd MMM yyyy")}
          </div>
        </div>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              {record.lab_results.map((l, i) => (
                ...
                >
                  ...
                  >
                    {l.result || l.value} {l.unit || ""}
                    {l.flag && l.flag !== "NORMAL" ? " ⚠️" : " ✓"}
                  </span>
                </div>
              ))}
            </div>
          ...
          {record.blockchain_tx_hash && (
            ...
            >
              ...
              <span style={{ fontFamily: "monospace" }}>
                {record.blockchain_tx_hash.slice(0, 20)}…
              </span>
            </div>
          )}

          <div style={{ marginTop: 12, fontSize: 11, color: "var(--text-3)" }}>
            Created: {safeFormat(record.created_at, "dd MMM yyyy HH:mm")}
          </div>
        </div>
      )}
    </div>
